...
#import "NSDate+KDUtilities.h"

@implementation NSDate (KDDateStyle)

- (NSString *)KD_stringByChineseRelativeFormatAllowFutureDate:(BOOL)allowFutureDate {
    int minutes = [self timeIntervalSinceNow] / 60;

    if (minutes < 0 || !allowFutureDate) {
        minutes = - minutes;
        if (minutes < 1) {
            return @"刚才";
        } else if (minutes < 60) {
            ...
            return [NSString stringWithFormat:@"%d小时前", minutes / 60];
        } else {
            return [NSString stringWithFormat:@"%d天前", minutes / 60 / 24];
        }
    } else {
        if (minutes < 1) {
            return @"现在";
            ...
            return [NSString stringWithFormat:@"%d小时后", minutes / 60];
        } else {
            return [NSString stringWithFormat:@"%d天后", minutes / 60 / 24];
        }
    }
}
...
@implementation NSDate (KDUtilities)
